    ...
    def execute(self):
        try:
            ...
            for datapoint in self.__market_data:
                MM_action = MomentumStrategy.generate_signals(self, datapoint, self.__position['AAPL']['quantity'], self.__position['AAPL']['avg_price'])
                MAC_action = MovingAverageCrossoverStrategy.generate_signals(self, datapoint, self.__position['AAPL']['quantity'], self.__position['AAPL']['avg_price'])
                if MM_action[0] != 'pass' or MAC_action[0] != 'pass':
                    ...
                    action = ('buy' if final_qty > 0 else 'sell', 'AAPL', abs(final_qty), datapoint.close)
                else:
                    action = ('pass', 'AAPL', 0, datapoint.close)
                ...
                if type(update) is list:
                    ...
                    self._position_hist = np.vstack((self._position_hist, [self.__position['AAPL']['quantity'], self.__position['AAPL']['avg_price']]))
