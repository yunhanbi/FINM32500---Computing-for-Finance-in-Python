...
class MomentumStrategy(Strategy):
    ...
    def generate_signals(self, tick, current_qty, avg_price) -> list:
        self._history_mm = np.append(self._history_mm, [[tick.high, tick.low, tick.close]], axis=0)
        if len(self._history_mm) < self._window:
            print('Waiting for more history')
            return ('pass', 'AAPL', 0, float(tick.close))
        vec = self._history_mm[-(self._window):]
        ...
        if adx[0] < self._adx_target:
            ...
            return ('buy', 'AAPL', int(self._risk // tick.close), float(tick.close))
        elif adx[1] == 'downtrend' and self.generate_rsi(vec) > self._rsi_target[1]:
            return ('sell', 'AAPL', int(current_qty), float(tick.close))
        else:
            return ('pass', 'AAPL', 0, float(tick.close))
...
class MovingAverageCrossoverStrategy(Strategy):
    ...
    def generate_signals(self, tick, current_qty, avg_price) -> list:
        self._history_mac = np.append(self._history_mac, tick.close)
